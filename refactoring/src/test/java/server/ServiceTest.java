...
public class ServiceTest {
    ...
    public static void startServer() throws Exception {
        ...
        properties.setProperty("database.url", "jdbc:postgresql://localhost:5432/products");
        properties.setProperty("database.username", "postgres");
        properties.setProperty("database.password", "321678eg");

        serverWeb = new ServerWeb(new Server(12345), properties);
        repository = new JPARepository(properties);
        serverWeb.start();
    }
    ...
    public void addingProductTest() throws URISyntaxException, IOException, InterruptedException {
        var request = createRequest("http://localhost:12345/add-product?name=svinyi&price=300");

        HttpResponse<String> response1 = client.send(request, HttpResponse.BodyHandlers.ofString());
        Assertions.assertEquals(200, response1.statusCode());

        request = createRequest("http://localhost:12345/add-product?name=pivo&price=1000");
    ...
    public void checkProductsTest() throws URISyntaxException, IOException, InterruptedException {
        assertByUrlAndBody(
                ...
                "<html><body>svinyi\t300</br>pivo\t1000</br></body></html>"
        );
    }
    ...
    public void checkQueryTest() throws URISyntaxException, IOException, InterruptedException {
        assertByUrlAndBody(
                ...
                "<html><body><h1>Product with max price: </h1>pivo\t1000</br></body></html>"
        );

        assertByUrlAndBody(
                "http://localhost:12345/query?command=min",
                "<html><body><h1>Product with min price: </h1>svinyi\t300</br></body></html>"
        );
        assertByUrlAndBody(
